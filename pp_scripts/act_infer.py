...
class ActAgent:
    ...
    @torch.no_grad()
    def predict_chunk(self, obs_img_hwc_uint8: np.ndarray, obs_state_18: np.ndarray) -> np.ndarray:
        """
        Args:
          obs_img_hwc_uint8: (H,W,3) uint8
          obs_state_18: (18,) float32/float64

        Returns:
          chunk: (T,7) float32
        """
        img = hwc_u8_to_chw_f01(obs_img_hwc_uint8).unsqueeze(0).to(self.device)  # [1,3,H,W]
        state = torch.from_numpy(obs_state_18.astype(np.float32)).unsqueeze(0).to(self.device)  # [1,18]
        ...
        #    如果 batch 里没有 "observation.images" 但有 overhead，就塞进去
        ...
        if isinstance(out, tuple):
            ...
            if actions_hat is None:
                raise KeyError(f"policy.model(batch) dict keys={list(out.keys())}, missing 'actions'/'action'")
        else:
            actions_hat = out
